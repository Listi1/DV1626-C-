...
{
    float num = GetBladeWidth();
    std::string num_str = std::to_string(num);

    std::string shoveldesc = ("Shovel, ");
    shoveldesc += num_str;
    shoveldesc += " m blade width, ";
    shoveldesc += Product::GetDescription();
 ...
 */
Shovel::Shovel(float price, float bladeWidth) : Product(float(price))
{   
...
{
    return 0.30f * 1.50f * this->bladeWidth;
}
...
{
    Shovel* clonedObject = new Shovel(*this);
    return clonedObject;
}
