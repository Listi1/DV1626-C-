...
std::string EggPackage::GetDescription() const
{
    ...
    int num = GetEggQuantity();
    std::string num_str = std::to_string(num);

    eggdesc += "Eggs,  ";
    eggdesc += num_str;
    eggdesc += "-pack, ";
    eggdesc += PerishableProduct::GetDescription();
...
EggPackage::EggPackage(float price, const Date& expirationDate, int eggQuantity)
    : PerishableProduct(float(price), Date(expirationDate))
{
...
{
    return 0.000125f * this->eggQuantity;
}
...
{
    EggPackage* clonedObject = new EggPackage(*this);
    return clonedObject;
}
